            ...
            match movement {
                ...
                ZoomMovement::Centered => {
                    ...
                    focal_point.x = focal_point.x.clamp(
                        ...
                        f64::next_down(output_geometry.loc.x + output_geometry.size.w),
                    );
                    focal_point.y = focal_point.y.clamp(
                        output_geometry.loc.y,
                        f64::next_down(output_geometry.loc.y + output_geometry.size.h),
                    );
            ...
            ZoomMovement::OnEdge => {
                    ...
                    .overlaps_or_touches(Rectangle::new(original_position, Size::from((16, 16))))
                {
                    ...
                    diff.x = diff.x.clamp(
                        ...
                        f64::next_down((output_geometry.loc.x + output_geometry.size.w) as f64),
                    );
                    diff.y = diff.y.clamp(
                        output_geometry.loc.y as f64,
                        f64::next_down((output_geometry.loc.y + output_geometry.size.h) as f64),
                    );
